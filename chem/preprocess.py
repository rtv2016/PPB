...
class lnKaScaler:
    ...
    def fit_transform(self, values):
        trimmed_array = [
            np.max([0.001, np.min([0.99, sample[0]])]) for sample in values
        ]
        y_temp = pd.Series(trimmed_array)
        transform_temp = y_temp / (1 - y_temp)
        transformed = 0.5 * transform_temp.apply(np.log)
        return transformed
    ...
    def transform(self, values):
        ...
        y_temp = pd.Series(trimmed_array)
        transform_temp = y_temp / (1 - y_temp)
        transformed = 0.5 * np.log(transform_temp)
        return transformed
...
class Reducer:
    ...
    def fit(self, data):
        self.nFeatures, self.aic, self.bic, self.rmse = get_n_features(
            data,
            self.nFeatures,
            self.yscaler,
            self.featSelect,
        ...
        self.featureList, self.featureRank = aggregate_feature_list(
            data, self.nFeatures, self.featSelect, self.featTypes, self.nSims
        )
        return self
...
def get_n_features(
    data, nFeatures, yscaler, featSelect, featTypes, modelType, nSims, verbose
):
    if not nFeatures:
        aic, bic, rmse = aic_curve(
            data, yscaler, featSelect, featTypes, modelType, nSims, verbose
        )
...
def aic_curve(
    ...
    numFeatures = (
        [1]
        + list(np.arange(5, len(train["X"][0]) / 10, 5))
    ...
    for n in numFeatures:
        ...
        for i in range(nSims):
            featureListAgg += list(
                find_features(train, n, featSelect, featTypes, feat_random_state=i)
            )
        ...
        preds = model_selection.cross_val_predict(
            model, train["X"][:, featureList], train["y"], cv=5, n_jobs=-1
        )
        if yscaler:
...
def find_features(
    data, nFeatures=10, featSelect="drugs", featTypes="RF", feat_random_state=4
):
    ...
    if featSelect.split("_")[0] == "predefined":
        ...
        X = data["X"][ind]
        y = data["y"][ind]
        if featTypes not in ["SVR", "RF", "Lasso"]:
            featTypes = "RF"
        ...
        if "SVR" in featTypes:
            ...
            grid = model_selection.GridSearchCV(
                sklearn.svm.LinearSVR(random_state=feat_random_state),
                {
                    "C": np.logspace(-2, 0, 3),
            ...
            model = sklearn.svm.LinearSVR(C=0.1, random_state=feat_random_state)
            clf["SVR"] = sklearn.feature_selection.RFE(model, nFeatures, 10).fit(
        ...
        if "Lasso" in featTypes:
            clf["Lasso"] = sklearn.feature_selection.RFE(
                sklearn.linear_model.Lasso(alpha=0.001), nFeatures
            ).fit(X[:endIndex], y[:endIndex])
            # Indices of top Lasso features
            return np.where(clf["Lasso"].ranking_ == 1)[0]
        # Random Forest
        if "RF" in featTypes:
            clf["RF"] = sklearn.ensemble.RandomForestRegressor(
                random_state=feat_random_state, n_estimators=250
            ).fit(X[:endIndex], y[:endIndex])
...
def sub_features(modelType):
    ...
    if modelType == "SVR":
        featuresList = [89, 175, 123, 154, 26, 8, 23, 161, 88, 121]
    else:
        featuresList = [89, 88, 154, 123, 73, 32, 65, 66, 69, 129]
