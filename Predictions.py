...
for i in range(1):  # range(1)
    model = chem.Modeler(modelType="KNN", random_state=i, verbose=1).fit(
        train_scaled["X"], train_scaled["y"]
    )
    for key in test_scaled:
        kNN_preds = model.predict(test_scaled[key]["X"])
        mae = metrics.mean_absolute_error(
            scaler.yscaler.inverse_transform(kNN_preds), test[key]["y"]
        )
...
for key in test_scaled:
    results[key] = []
for i in range(1):  # range(1)
    model = chem.Modeler(modelType="SVR", random_state=i, verbose=1).fit(
        train_scaled["X"], train_scaled["y"]
    )
    for key in test_scaled:
        SVR_preds = model.predict(test_scaled[key]["X"])
        mae = metrics.mean_absolute_error(
            scaler.yscaler.inverse_transform(SVR_preds), test[key]["y"]
        )
        results[key].append(mae)
for key in results:
    print(key, np.mean(results[key]), np.std(results[key]))
...
for key in test_scaled:
    results[key] = []
for i in range(1):  # range(1)
    model = chem.Modeler(modelType="RF", random_state=i, verbose=1).fit(
        train_scaled["X"], train_scaled["y"]
    )
    for key in test_scaled:
        RF_preds = model.predict(test_scaled[key]["X"])
        mae = metrics.mean_absolute_error(
            scaler.yscaler.inverse_transform(RF_preds), test[key]["y"]
        )
        results[key].append(mae)
for key in results:
    print(key, np.mean(results[key]), np.std(results[key]))
